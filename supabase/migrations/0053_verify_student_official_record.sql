-- Official Academic Record: richer public student identity payload (tenant-scoped)
-- Adds avatar, program/course, academic status; optional subdomain filter.

drop function if exists public.verify_student_identity(text);
...
comment on function public.verify_student_identity(text, text) is
  'Public Official Academic Record lookup. Optional p_subdomain scopes to one tenant.';
