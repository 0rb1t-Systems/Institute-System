-- When an imported certificate template is updated (e.g. patch uploaded),
-- republish it to the live custom_upload.design so generate/view pick up images.
...
begin
  ...
  -- Always republish so patch/logo changes appear on student generate/view/download
  ...
  if nullif(trim(coalesce(v_upload->>'storage_path', '')), '') is null then
    v_upload := v_upload || jsonb_build_object(
      ...
      'file_name', left(trim(coalesce(v_row.source_file_name, v_name)), 180),
      'mime_type', case
        when v_row.source_kind = 'docx' then 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
...
comment on function public.update_imported_certificate_template(uuid, text, jsonb, uuid, jsonb) is
  'Update an imported certificate template and republish it to custom_upload.design for student generation.';
