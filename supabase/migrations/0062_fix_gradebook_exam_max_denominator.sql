-- =============================================================================
-- 0062_fix_gradebook_exam_max_denominator.sql
-- Only count exams that already have a result for the student when computing
-- exam_max. Previously ungraded exams inflated the denominator
-- (e.g. score 82 / max 200 → 41% F instead of 82% B).
-- =============================================================================
...
begin
  ...
  -- Only exams with a graded result for this student contribute to score/max
  select
...
-- Keep bonus cap aligned: exam max only from graded exams
...
-- Recompute auto gradebook entries with the corrected denominator
do $$
declare
  r record;
begin
  for r in
    select distinct ge.student_id, ge.class_id, ge.course_id, ge.institution_id, ge.enrollment_id
...
comment on function public.resync_gradebook_for_student_course(uuid, uuid, uuid, uuid, uuid) is
  'Final = min(exam_max, exam_score + assignment_bonus) as % of graded-exam max only.';
