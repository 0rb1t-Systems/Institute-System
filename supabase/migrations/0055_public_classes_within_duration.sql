-- Online registration: only active classes currently within their duration window
...
begin
  ...
  where c.institution_id = v_inst_id
    and c.status = 'active'
    and c.start_month is not null
    and c.end_month is not null
    and v_month between date_trunc('month', c.start_month)::date
                    and date_trunc('month', c.end_month)::date;
...
-- Same duration gate when submitting an inquiry (cannot bypass via API)
...
begin
  ...
  if p_class_id is not null then
    ...
    where id = p_class_id
      ...
      and v_month between date_trunc('month', start_month)::date
                      and date_trunc('month', end_month)::date
    limit 1;
    if not found then
