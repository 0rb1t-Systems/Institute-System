    ...
    <div className={dashboardStyles.section}>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
        ...
        <Button 
          ...
          className="border-[var(--tenant-line)] text-[var(--tenant-text)] hover:bg-[var(--tenant-bg-2)]"
        >
          Manage All Classes
        </Button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[var(--tenant-line)]">
              {sortedClasses.map((cls) => (
                <tr key={cls.id} className="group hover:bg-[color-mix(in_srgb,var(--brand-primary)_5%,transparent)] transition-colors">
                  <td className="p-4 pl-6">
                    <div>
                      <h4 className="font-medium text-[var(--tenant-text)] transition-colors">
                        {cls.name}
                      </h4>
                      <p className="text-xs text-[var(--tenant-muted)] mt-1">{cls.course?.name || 'General Course'}</p>
                    </div>
                  </td>
                  <td className="p-4">
                    <div className="text-sm text-[var(--tenant-muted)]">
                  ...
                  <td className="p-4">
                    <div className="w-[140px]">
                      <div className="flex justify-between text-xs mb-1.5">
                        <span className="text-[var(--tenant-text)] font-medium">{cls.enrollmentCount} Students</span>
                        <span className="text-[var(--tenant-muted)]">{Math.round(cls.fillRate)}%</span>
                      </div>
                      <Progress value={Math.min(100, Math.max(0, cls.fillRate))} className="h-1.5 bg-[var(--tenant-line)]" />
                    </div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right">
                    ...
                    <div className="text-[10px] text-[var(--tenant-muted)] mt-0.5">
                  ...
                  <td className="p-4 text-right pr-6">
                    <Button 
                      size="sm" 
                      variant="ghost" 
                      className="h-8 w-8 p-0 text-[var(--tenant-muted)] hover:text-[var(--tenant-text)] hover:bg-[var(--tenant-bg-2)] rounded-full"
                      onClick={() => navigate(`/instructor/classes`)}
                    >
              ...
              {sortedClasses.length === 0 && (
                <tr>
                  <td colSpan={6} className="p-12 text-center text-[var(--tenant-muted)]">
