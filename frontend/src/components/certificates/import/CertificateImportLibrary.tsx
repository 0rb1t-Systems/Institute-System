...
import type { SavedCertificateImport } from '@/lib/certificateImport/api'
...
type Props = {
  items: SavedCertificateImport[]
  loading?: boolean
  activeId?: string | null
  activatingId?: string | null
  institution: InstitutionBrand | null
  onEdit: (item: SavedCertificateImport) => void
  onDelete: (id: string) => void
  onActivate: (item: SavedCertificateImport) => void
}
...
const CertificateImportLibrary = ({
  items,
  loading,
  activeId,
  activatingId,
    ...
    return (
      <div className="rounded-2xl border border-dashed border-[var(--ds-border,#DDE5DF)] bg-[var(--ds-surface-muted,#F7FAF8)] px-6 py-10 text-center">
        ...
        <p className="mt-1 text-sm text-[var(--ds-text-secondary,#5B6B61)]">
          Upload a PDF or DOCX above, name it, pick a class, then save. You can keep many templates here.
        </p>
      </div>
    )
  }
...
export default CertificateImportLibrary
